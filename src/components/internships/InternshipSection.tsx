...
export default function InternshipSection() {
  ...
  const openModal = useCallback((cred: { src: string; title: string; alt: string }) => {
    setModalImage(cred)
  }, [])
  ...
  const cardVariants: Variants = {
    ...
    show: {
      opacity: 1,
      y: 0,
      transition: reduced ? { duration: 0 } : { duration: 0.8, ease: ease.paper },
    },
  }
      ...
      >
        <div className="relative z-10 mx-auto max-w-[112rem] px-[max(1.5rem,7vw)]">
          ...
          <div className="mb-[clamp(2rem,5vw,3.5rem)] flex items-baseline justify-between border-b border-ink/20 pb-4">
            ...
            <span className="font-mono text-[0.8rem] text-graphite hidden sm:inline">
              INFOSYS_SPRINGBOARD_6.0.SYS
            </span>
          </div>

          {/* Featured Internship Card — Preserved Layout */}
          {exp && (
            <motion.article
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-12 lg:gap-8 items-stretch">
                ...
                <div className="flex flex-col justify-between lg:col-span-7">
                  <div>
                    ...
                    <p className="body-copy m-0 mt-3.5 text-[0.95rem] leading-relaxed text-graphite font-mono">
                      {exp.description}
                    </p>
                    ...
                    <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-3 border-y border-ink/15 py-3.5">
                      <div>
                        <p className="eyebrow m-0 font-mono text-[0.65rem] tracking-wider text-graphite">
                          PROGRAM / BATCH
                        </p>
                    ...
                    <div className="mt-5">
                      <p className="eyebrow m-0 mb-2 font-mono text-[0.68rem] tracking-wider text-ink/70">
                        CURRICULUM &amp; VALIDATED TRACKS
                      </p>
                      <div className="flex flex-wrap gap-1.5 font-mono text-[0.72rem]">
                        {exp.tags.map((tag) => (
                          <span
                            key={tag}
                            className="rounded-[3px] border border-ink/15 bg-white/80 px-2 py-1 font-medium text-ink/90 shadow-2xs"
                          >
                            #{tag}
                          </span>
                        ))}
                  ...
                  <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-ink/10 pt-4">
                    {exp.credentials.map((cred) => (
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-full border border-ink bg-ink px-4 py-2 font-mono text-[0.75rem] font-bold text-paper transition-all duration-200 hover:bg-signal hover:border-signal shadow-xs"
                      >
                        <span>VIEW {cred.title.toUpperCase()} ↗</span>
                      </button>
                    ))}
                    <a
                      href="https://springboard.infosys.com"
                      target="_blank"
                    ...
                    >
                      <span>INFOSYS PORTAL ↗</span>
                    </a>
                  </div>
                </div>

                {/* Right Visual / Engineering Console Column — Preserved */}
                <div className="flex flex-col justify-between overflow-hidden rounded-[0.5rem] border border-ink/30 bg-[#121211] text-paper shadow-md lg:col-span-5 min-h-[320px]">
                  ...
                  <div className="relative flex-1 p-4 sm:p-5 font-mono text-[0.74rem] bg-[#0d0d0c] text-white/90 flex flex-col justify-center">
                    ...
                    <pre className="m-0 overflow-x-auto font-mono text-[0.72rem] leading-relaxed text-white/85">
                      <code>
                        <span className="text-white/40"># Verified Practicum &amp; Track Record</span>{'\n'}
                        <span className="text-[#f4b400]">institution</span>  = <span className="text-[#0f9d58]">&quot;Infosys Springboard&quot;</span>{'\n'}
                        <span className="text-[#f4b400]">program</span>      = <span className="text-[#0f9d58]">&quot;Virtual Internship 6.0&quot;</span>{'\n'}
                        <span className="text-[#f4b400]">batch</span>        = <span className="text-[#0f9d58]">&quot;Batch 7 (Commenced Nov 2025)&quot;</span>{'\n'}
                        <span className="text-[#f4b400]">curriculum</span>   = [{'\n'}
                        {'    '}<span className="text-[#4285f4]">&quot;Core Python &amp; Algorithms&quot;</span>,{'\n'}
                        {'    '}<span className="text-[#4285f4]">&quot;Object-Oriented Design&quot;</span>,{'\n'}
                        {'    '}<span className="text-[#4285f4]">&quot;Software Automation&quot;</span>{'\n'}
                        ]{'\n'}
                        <span className="text-[#f4b400]">status</span>       = <span className="text-signal">&quot;Selected &amp; Enrolled&quot;</span>
                      </code>
                    </pre>
                    <div className="mt-3.5 flex items-center justify-between rounded border border-white/10 bg-white/5 px-2.5 py-1.5 text-[0.66rem] text-white/60">
                      ...
                      <span className="text-signal font-bold">SELECTION CONFIRMED</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5 p-3.5 font-mono text-[0.72rem] text-white/80 bg-[#141413] border-t border-white/10">
                    <p className="m-0 flex justify-between">
                      <span className="text-white/40">Training Portal:</span>
                      <span>Infosys Springboard</span>
                    </p>
                    <p className="m-0 flex justify-between">
                      <span className="text-white/40">Competency:</span>
                      <span>Programming &amp; Software Design</span>
                    </p>
                    <p className="m-0 flex justify-between">
                      <span className="text-white/40">Credential Files:</span>
                      <span className="text-signal font-bold">Selection Letter &amp; Badge</span>
                    </p>
                  </div>
                </div>
              </div>
          ...
          >
            ...
            >
              ...
              <div className="border-t border-white/10 bg-[#161614] px-4 py-2 text-center font-mono text-[0.68rem] text-white/60">
                Official Credential Document · Press ESC or click outside to dismiss
              </div>
            </div>
          </motion.div>
        )}
